...
Create `backend/.env` from `backend/.env.example` and set the database and JWT values:
...
Run the seed command before using these accounts. The same password is used for
all accounts in each role.
...
| 🧑‍🎓 Student | deepa@student.edu    | student123 |
